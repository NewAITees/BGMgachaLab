...
新しいジャンルを円環に差し込める。画面下部の「ジャンル追加」ボックスから
自由入力するとこの形式のファイルが自動生成される（後述）。
...
- ジャンル数の上限は**12個**（`MAX_GENRES`）。上限に達した状態でさらに差し込むと、
...
`pending_genres/` に書き込む（実体は `genre_interpreter.py`）。

- 実在の作曲家/アーティスト名を入力した場合も、本人の楽曲を模倣するのではなく
  「一般的な音楽的特徴（楽器編成・雰囲気・テンポ）」として解釈させている
- BPMは70〜190の範囲にクランプされる
- 解釈結果はその場では円環に反映されず、上記の差し込み機構（`insert_pending_genres`）が
  次回の生成タイミングで拾って挿入する
